...
implementation {
  ...
  uint8_t discarded [3];      // Tracking the last ACK success/fail of different priorities
...
#define FAULT_LENGTH_SLOTS 15                      // The length of the fault in slots
  ...
  void mcc_check_to_go_high() {
	...
	if (node_ack_fails >= CRIT_CHANGE_THRESHOLD) { 
	  ...
	  discarded[0] += call forwardQueue.sizeLO_Q1();
	  discarded[1] += call forwardQueue.sizeLO_Q2();
	  discarded[2] += call forwardQueue.sizeLO_Q3();
  ...
  void handle_tx_slot(uint8_t slot) {
	...
	atomic {
	  ...
	  dataPacket->data[0] = discarded[0];
	  dataPacket->data[1] = discarded[1];
	  dataPacket->data[2] = discarded[2];
	  discarded[0] = 0;
	  discarded[1] = 0;
	  discarded[2] = 0;
	}
  ...
  command error_t MacSend.send(message_t * msg, uint8_t len) {
    ...
    if ((hiCritMode == TRUE) && (packetToEnqueue->crit_level == LO)) { // NA
      ...
      // Record the discarding - since we are measuring total frames here
      // use c to determine how many frames discarded
      if (packetToEnqueue->priority == 1) { discarded[0] += packetToEnqueue->c; }
      if (packetToEnqueue->priority == 2) { discarded[1] += packetToEnqueue->c; }
      if (packetToEnqueue->priority == 3) { discarded[2] += packetToEnqueue->c; }
