...
// Original author: A. Ajith Kumar S. 

// * author:      James Harbin modified the following component from the MCC code
...
// Need to go back to LO on having an empty buffer, even if we are in the LO mode
#include <printf.h>
...
#define FAULT_INTERVAL 30
  uint8_t fault_slot_countdown = FAULT_INTERVAL;
  ...
  void mcc_check_to_go_high() {
    ...
    // Could this be an off-by-one here?
    if (hiCritMode == FALSE) {
  ...
  void handle_tx_slot(uint8_t slot) {
    ...
    // (since it cannot have any effect here)
    if (queue_has_pkt == FAIL) {
    ...
    // It has now been removed. Watch for the
	...
	mcc_printf(forwardDataPacket);
      ...
      // dataPacket->failed_ack_stats = (node_ack_fails & 0x0F) + ((max_node_ack_fails << 4) & 0xF0);
	...
	#define FLOW_1_DEADLINE 25

	// Debuggging code for tracking timing errors in flow 1
	if (TOS_NODE_ID == 1) {
	  if (dataPacket->priority == 2) {
	    if ((dataPacket->hopSendSlot - localSlot) > FLOW_1_DEADLINE) {
	      printf("Flow 1 frame exceeded deadline: ");
	      mcc_printf(dataPacket);
	      printf("\n");
	    }
	  }
	}
  ...
  async event void Slotter.slot(uint8_t slot) {
    ...
    atomic {
      ...
      fault_slot_countdown = fault_slot_countdown - 1;

      // Trigger the fault if ready
      if (fault_slot_countdown == 0) {
	mcc_fault_triggered();
	fault_slot_countdown = FAULT_INTERVAL;
      }
    }
...
#ifdef UPDATE_LOCAL_SLOT_FROM_SINK
