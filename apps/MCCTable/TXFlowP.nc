...
implementation {
  ...
  void setup_mcc_app_packet(mccpacket * mccpkt) {
    DPRINTF0("APP: Setting up MCC packet fields\n");
    ...
    DPRINTF("MCC packet src = %u, prio = %u, destination = %u\n", mccpkt->src, mccpkt->priority, mccpkt->dst);
  }

  uint32_t ms_timer() {
    return ((uint32_t)periodSlots) * SLOT_LENGTH_FREQUENCY_CORRECTED;
  }

  event void Boot.booted() {
    DPRINTF("TXFlowP: booting flow %u...\n", flow_num);
    printfflush();
    packetsSent = 0;
    //call SendTimer.startOneShot(FLOW_INIT_TIMER + ((uint32_t)periodSlots * flow_num));
    call SendTimer.startOneShot(FLOW_INIT_TIMER + ((uint32_t)SLOT_LENGTH_FREQUENCY_CORRECTED * flow_num));
    return;
  }

  event void SendTimer.fired() {
    DPRINTF("TXFlowP: Timer fired - busy is %d\n", busy);
  ...
  task void send() {
    // FIX: change this to use a seperate abstraction for application layer messages,
    ...
    DPRINTF("TXFlowP.send flow %u: seq_num = %u\n", flow_num, mccpkt->seq_num);
    if(call AppSend.send(&packet, sizeof(mccpacket)) == SUCCESS) {
      busy = TRUE;
      DPRINTF0("TXFlowP: Pkt passed to APP\n");
    } else {
      DPRINTF0("TXFlowP: Packet failed\n");
    }
  }
  ...
  event void AppSend.sendDone(message_t * msg, error_t error) {
    ...
    DPRINTF("TXFlowP: Packets sent:%d\n", packetsSent);
  }
}