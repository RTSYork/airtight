// For load testing - listener node constantly listens

// Todo: node 1 transmits to node 2 - every slot
// Then node 2 replies back - every slot
// Each node counts each other's heard transmissions and appends the counter into the packet
// If a duplicate count detected, that's a missed reception by the other
...
#define FAULT_LENGTH_SLOTS 15                      // The length of the fault in slots
  ...
  void print_mcc_action(action a) {
    switch (a) {
    case A_IDLE: DPRINTF0("A_IDLE\n"); break;
    case A_TX: DPRINTF0("A_TX\n"); break;
    case A_LISTEN: DPRINTF0("A_LISTEN\n"); break;
    }
  }
  ...
  void setup_temp_mcc_table() {
	...
	atomic mccSchedTable[slot] = A_LISTEN;
      ...
      atomic {
	if (TOS_NODE_ID == 1) mccSchedTable[2] = A_TX;
	if (TOS_NODE_ID == 0) mccSchedTable[3] = A_TX;
	if (TOS_NODE_ID == 1) mccSchedTable[4] = A_TX;
	if (TOS_NODE_ID == 0) mccSchedTable[5] = A_TX;
      }      
  ...
  void handle_tx_slot(uint8_t slot) {
    ...
    // For load testing, we always have a packet present in the queue

    // Set parameters here for load testing
    forwardDataPacket->flow_id = TOS_NODE_ID;
    if (TOS_NODE_ID == 0) forwardDataPacket->dst = 1;
    if (TOS_NODE_ID == 1) forwardDataPacket->dst = 0;
	...
	atomic {
	  ...
	  dataPacket->data[1] = reception_counter & 0xFF;
	  dataPacket->data[0] = (reception_counter >> 8) & 0xFF;
	}
  ...
  event void phyDataSend.sendDone(message_t *msg, error_t error) {
    ...
    if (call ACK.wasAcked(msg) && (activeFault == FALSE)) {
