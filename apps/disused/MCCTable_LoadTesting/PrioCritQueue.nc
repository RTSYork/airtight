...
interface PrioCritQueue<t> {
  ...
  // The number of LO crit packets currently in the queue
  command uint8_t sizeLO();

  // The number of LO crit packets currently in the particular numbered queue
  command uint8_t sizeLO_Q1();
  command uint8_t sizeLO_Q2();
  command uint8_t sizeLO_Q3();
}
