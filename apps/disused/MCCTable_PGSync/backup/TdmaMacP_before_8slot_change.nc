...
#define MCC_LO_CRIT_RT_LIMIT 1
  ...
  void setup_temp_mcc_table() {
	...
	atomic mccSchedTable[slot] = A_LISTEN;
      ...
      if (TOS_NODE_ID == 4) mccSchedTable[7] = A_TX;
      }
  ...
  async event void Slotter.slot(uint8_t slot) {
	...
	// Other periodic sink debugging information here
      DPRINTF0("<SINK>: Packets received until now \n");
      DPRINTF("1: %d, 2:%d, 3:%d, 4:%d \n",
	     nbReceivedPkts[1], nbReceivedPkts[2],nbReceivedPkts[3],nbReceivedPkts[4]);
    }
