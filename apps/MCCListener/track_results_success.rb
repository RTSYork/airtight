...
def log_pkt_to_file(tdiff, resfile, pkt)
  resfile.print "#{lz(pkt[:flow_id], 2)},#{lz(pkt[:seq_num], 5)},#{lz(pkt[:retrans_count],1)},#{lz(pkt[:response_time], 2)},#{pkt[:burst_num]},#{pkt[:failed_ack_count]}\n"
  resfile.flush
end
...
def track_flow_success(current_success, flow_id)
  ...
  # check each element of current_success. resolve for the
  ...
  [1,2,3].each do |i|
    ...
    if (not flow_ids[i].nil?) && (flow_ids[i] >= 1 && flow_ids[i] < 12) then
      puts "current_success = #{current_success.join(",")}"
      @packet_count_for_flows[flow_ids[i]] += current_success[i]
    end
  end
end
...
def decode_pkt(dec, del_file)
  ...
  current_success = [ nil, dec[-3], dec[-2], dec[-1] ]
  track_flow_success(current_success, flow_id);

  ratio = div_array_one_based(@packet_count_for_flows, @sent_count_for_flows)
  
  puts "Packet count for flows - #{@packet_count_for_flows.join(",")} - #{@sent_count_for_flows.join(",")} - success ratios #{ratio.join("\t")}"

  del_file.print "#{ratio.join(",")}\n"
  del_file.flush
...
def load_info(cmd, debug_print)
  res_file = "pkt-res-#{date_str}"
  delsuccess_file = "delivery-success-#{date_str}"

  File.open("results/#{res_file}", "a") do |res|
    File.open("results/#{delsuccess_file}", "a") do |delfile|
